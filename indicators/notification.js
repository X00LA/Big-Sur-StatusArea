...
// Icon-Pfade werden per setExtensionPath() gesetzt (aus extension.js)
let _extensionPath = '';
export function setExtensionPath(p) { _extensionPath = p; }
...
class CalendarColumnLayout2 extends Clutter.BoxLayout {
    ...
    vfunc_get_preferred_width(container, forHeight) {
        const actors = this._colActors.filter(a => a.get_parent() === container);
        if (actors.length === 0)
...
export var NotificationIndicator = GObject.registerClass({
...
class NotificationIndicator extends CustomButton {
    ...
    _init() {
        ...
        this._settingsChanged = this.settings.connect(
            'changed::separate-date-and-notification',
            () => this.applySettings()
        );
        ...
        try { this._messageList._removeSection(this._messageList._eventsSection); } catch (_) {}
        ...
        this.hbox = new St.BoxLayout({ vertical: false, name: 'calendarArea' });

        this._vbox = new St.BoxLayout({ height: 400 });
        ...
        try { this._messageList._removeSection(this._messageList._mediaSection); } catch (_) {}
    ...
    setHide(value) {
        ...
        if (!value)
            this.actor.show();
        else if (this._indicator._sources.length === 0)
            this.actor.hide();
    }
    ...
    prepareCalendar() {
        if (!this.settings.get_boolean('separate-date-and-notification')) {
            ...
            this._eventsSection  = new DateMenu.EventsSection();
    ...
    removeCalendar() {
        if (!this.settings.get_boolean('separate-date-and-notification')) {
            ...
            try { this.displayBox.remove_child(this._clocksSection); } catch (_) {}
            try { this.displayBox.remove_child(this._weatherSection); } catch (_) {}
            try { this._vboxd.remove_actor(this._date); } catch (_) {}
            try { this._vboxd.remove_actor(this._calendar); } catch (_) {}
            try { this.box.remove_child(this._clockIndicator); } catch (_) {}
    ...
    destroy() {
        ...
        this.settings.disconnect(this._settingsChanged);
        this.removeCalendar();
        this._indicator.destroy();
        try { this._vbox.remove_child(this._messageList); } catch (_) {}
...
class MessagesIndicator extends GObject.Object {
    ...
    _init(src, settings) {
        ...
        const noNotif   = `${_extensionPath}/icons/notification-symbolic.svg`;
        const newNotif  = `${_extensionPath}/icons/notification-new-symbolic.svg`;
        const disNotif  = `${_extensionPath}/icons/notification-disabled-symbolic.svg`;

        this._Notifications_gicon    = Gio.icon_new_for_string(noNotif);
        this._NewNotifications_gicon = Gio.icon_new_for_string(newNotif);
        this._DisabledNotifications_gicon = Gio.icon_new_for_string(disNotif);

        this._desktopSettings = new Gio.Settings({
            schema_id: 'org.gnome.desktop.notifications',
        });
        this._desktopSettings.connect('changed::show-banners', () => this._updateCount());
    ...
    _updateCount() {
        let count = 0;
        this._sources.forEach(source => { count += source.count; });

        if (this.settings.get_boolean('separate-date-and-notification')) {
            if (!this._desktopSettings.get_boolean('show-banners'))
                this._icon.gicon = this._DisabledNotifications_gicon;
            else
